...
- Observed: member creation no longer fails on `500` after successful inserts because the audit-log repository now handles nullable JSONB (`old_value`/`new_value`) safely, duplicate email/username database conflicts map more defensively to friendly messages, global toast feedback renders above action sheets, and the member modal stays visually centered.
