...
- Observed: app-shell navigation now flips to the tapped destination immediately, preloads route code from persistent nav surfaces, and uses lighter intro-only motion so navbar/sidebar menu changes no longer feel delayed while waiting on the full route swap.
