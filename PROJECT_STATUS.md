...
- Observed: app-shell navigation now flips to the tapped destination immediately, preloads route code from persistent nav surfaces, uses lighter intro-only motion so navbar/sidebar menu changes no longer feel delayed while waiting on the full route swap, and turns the mobile overflow menu into a compact two-column panel under the mobile header that only lists non-navbar destinations with icon-first tiles.
