...
<div class="space-y-4">
  {#if $navigating?.to?.url.pathname === '/dashboard'}
    <StatePanel
      tone="loading"
      title="Memuat"
      message="Memuat ulang ringkasan operasional mess..."
    />
  {/if}
  ...
  >
    ...
    <div class="mt-4 grid gap-3 sm:grid-cols-3">
      <a href="/members" class="stat-card bg-white transition hover:border-sky-300 hover:bg-sky-50/50">
        ...
        <p class="mt-2 text-base font-semibold text-ink">Anggota</p>
  ...
  >
    ...
    {:else if data.leaderboardSummary.state === 'empty'}
      <StatePanel tone="empty" title="Kosong" message={data.leaderboardSummary.message ?? 'Belum ada kontribusi bulan ini'} />
    {:else}
      <StatePanel tone="error" title="Gagal memuat" message={data.leaderboardSummary.message ?? 'Data kontribusi belum tersedia.'} />
    {/if}
  </PageCard>
    ...
    >
      <div class="grid gap-3 md:grid-cols-2">
        {#each adminQuickLinks as link}
          <a href={link.href} class="stat-card bg-white transition hover:border-sky-300 hover:bg-sky-50/50">
